...
<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0" text-align=center>
...
This phenomenon of how the alignment between the model and the data generating process affect the accuracy and the precision of the model is the classic case of statistical bias-variance trade-off. It is important to note the discussion on bias-variance trade-off in popular machine learning literature on the web attributes the difference in the prediction and the ground truth to the capacity of the model. However, this argument is incomplete as we must take the data generating process into account. A more capacious model does not have to exhibit low bias. Also, modern machine learning theory tells us more capacious model can increase the precision as well as the accuracy further when the models are highly overparameterized. Finally, statistical bias is just a part of the story. There are other forms of bias present in machine learning algorithms that I intend to cover in one of the subsequent articles.
